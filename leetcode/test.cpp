	...
	vector<vector<string>> solveNQueens(int n) {
		...
		for (int i = 0; i < (n + 1) / 2; i++) {
			...
			while (k > 0) {
				if (k == n) {
					res.emplace_back();
					res.back().resize(n, templ);
					for (int r = 0; r < n; r++)
						res.back()[r][board[r].pos] = 'Q';
					if (i < n / 2) {
						res.emplace_back();
						res.back().resize(n, templ);
						for (int r = 0; r < n; r++)
							res.back()[r][n-1-board[r].pos] = 'Q';
					}
					k--;
					continue;
				}
...
void
check(int arg, vector<vector<string>> res)
{
	Solution s;
	auto my = s.solveNQueens(arg);
	if (my != res) {
		std::cout << "expected:\n";
